  ...
  const getData = async () => {
    ...
    Promise.all([
      ...
      for (let i = 0, j = 0; i < defSuperTokenList.length; i++) {
        ...
        defSuperTokenList[i].woofMinOut = fromWei(minCowoof, WOOF.decimals).toFixed(4)
        defSuperTokenList[i].woofMin = fromWei(res2[i] || 0, defSuperTokenList[i].decimal).toFixed(4)
        if (i >= 1) {
